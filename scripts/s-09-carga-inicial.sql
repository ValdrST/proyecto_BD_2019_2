...
begin
    ...
    insert into aparato(aparato_id,numero_serie,numero_matricula,codigo_acceso,capacidad,porcentaje_carga,estado_id,recarga_id,marca_id) 
        values (aparato_seq.nextval,'798217392873927323','764NNU','98173216492163921512374293715423',100.23,80,v_estado(1),v_servicio(1),v_marca(1));
    v_aparato.extend;
    v_aparato(1) := aparato_seq.currval;
    insert into aparato(aparato_id,numero_serie,numero_matricula,codigo_acceso,capacidad,porcentaje_carga,estado_id,recarga_id,marca_id) 
        values (aparato_seq.nextval,'798217392123927321','765NNU','98173216492163921242671293715422',78.23,80,v_estado(2),v_servicio(1),v_marca(2));
    v_aparato.extend;
    v_aparato(2) := aparato_seq.currval;
    insert into aparato(aparato_id,numero_serie,numero_matricula,codigo_acceso,capacidad,porcentaje_carga,estado_id,recarga_id,marca_id) 
        values (aparato_seq.nextval,'798217457287392732','766NNU','98173216492163543512371293715424',99.23,80,v_estado(2),v_servicio(1),v_marca(3));
    v_aparato.extend;
    v_aparato(3) := aparato_seq.currval;
    insert into usuario(usuario_id,email,nombre,apellidos,contraseña,puntos,es_socio)
    ...
    insert into tarjeta(usuario_id,numero,mes_expiracion,año_expiracion)
        values (usuario_seq.currval,'1233456719023221','03','19');
    v_usuario.extend;
    ...
    v_usuario.extend;
    v_usuario(3) := usuario_seq.currval;
    insert into servicio(servicio_id,usuario_id,tipo) 
        values (servicio_seq.nextval,usuario_seq.currval,'R');
    v_servicio.extend;
    v_servicio(3) := servicio_seq.currval;    
    ...
    insert into ubicacion(ubicacion_id,latitud,longitud,fecha_hora,aparato_id)
        values (ubicacion_seq.nextval,130.000000,30.002300,sysdate,v_aparato(3));
    insert into zona_aparato(zona_aparato_id,zona_id,aparato_id)
        values (zona_aparato_seq.nextval,v_zona(3),v_aparato(1));
    insert into zona_aparato(zona_aparato_id,zona_id,aparato_id)
        values (zona_aparato_seq.nextval,v_zona(2),v_aparato(2));
    insert into zona_aparato(zona_aparato_id,zona_id,aparato_id)
        values (zona_aparato_seq.nextval,v_zona(3),v_aparato(3));
end;
/