...
create or replace procedure p_exporta_datos_usuario(p_usuario_id in numeric)
is
...
begin
  v_file := utl_file.fopen('DATA_DIR','Reporte-'||p_usuario_id,'w');
  ...
  from usuario
  where usuario_id=p_usuario_id;
    utl_file.put_line(v_file,'es socio: '||v_es_socio);
